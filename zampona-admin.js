...
function getTubeAt(row,pos){
  return tubes.find(t=>t.fila===row && Number(t.posicion)===Number(pos));
}

function renderAdminMap(){
  const upper = $("#adminUpperRow");
  const lower = $("#adminLowerRow");
  ...
  defaultTubeRows().forEach(slot=>{
    ...
    btn.addEventListener("click",()=>{
      if(existing) fillTube(existing);
      else fillEmptySlot(slot);
    });
...
function renderTubeList(){
  ...
  defaultTubeRows().forEach(slot=>{
    ...
    const title = existing
      ? `${slot.fila==="superior" ? "S" : "I"}${slot.posicion} · ${existing.etiqueta || existing.numero || "sin etiqueta"}`
      : `${slot.fila==="superior" ? "S" : "I"}${slot.posicion} · sin configurar`;

    btn.innerHTML = `<strong>${title}</strong><span>${existing?.nota || "Sin nota"}${existing?.frecuencia ? " · "+existing.frecuencia+" Hz" : ""}</span>`;
...
function fillTube(tube){
  ...
  $("#tubeNote").value = tube.nota || "";
  $("#tubeFrequency").value = tube.frecuencia || "";
...
function playFrequency(freq){
  ...
  if(!value || value<=0){
    setStatus($("#tubeSaveStatus"),"Ingresa una frecuencia válida para escuchar el tubo.","error");
    return;
  }
...
$("#playTubeBtn").addEventListener("click",()=>{
  playFrequency($("#tubeFrequency").value);
});
...
$("#saveTubeBtn").addEventListener("click",async()=>{
  ...
  const payload = {
    ...
    nota:$("#tubeNote").value.trim(),
    frecuencia:$("#tubeFrequency").value ? Number($("#tubeFrequency").value) : null,
  ...
  let query;
  if(id){
    query = window.CancioneroDB.client
      .from("zampona_tubos")
      .update(payload)
      .eq("id",id);
  }else{
